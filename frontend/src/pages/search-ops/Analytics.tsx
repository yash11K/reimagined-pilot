...
// DEMO ONLY — backend has no search-analytics endpoint yet.
    ...
    <>
      <div className="mb-3 flex items-center justify-between">
        <div className="inline-flex items-center gap-2 rounded-lg border border-status-warn/30 bg-status-warnSoft px-3 py-1.5 text-xs font-medium text-status-warn">
          ...
          Demo data — no analytics endpoint wired yet.
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Total Searches" value="12,847" trend={{ value: "+12.4% vs last period", direction: "up" }} icon={SearchIcon} fake />
        <KpiCard label="Avg Success Rate" value="91.6%" trend={{ value: "+2.1pp", direction: "up" }} icon={TrendingUp} fake />
        <KpiCard label="Avg Response Time" value="0.35s" trend={{ value: "-0.05s", direction: "up" }} icon={Clock} fake />
        <KpiCard label="Failed Queries" value="1,079" trend={{ value: "-8.2%", direction: "up" }} icon={AlertTriangle} fake />
      </div>
